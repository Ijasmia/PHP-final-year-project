...
if(isset($_POST)) {
	...
	$sql = "SELECT id_user, firstname, lastname, email, chat_rank, usertype, phone, whatsapp, active FROM users WHERE email='$email' AND password='$password'";
	...
	if($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
 		...
 		$_SESSION['loginError'] = $link->error;
 		header("Location: login.php");
		exit();
 	}
